...
public class EventLogListener {
    ...
    public void eventListenerDefault(EventDTO event,
                                   @Header(KafkaHeaders.OFFSET) long offset,
                                   @Header(KafkaHeaders.RECEIVED_PARTITION) int partition){

        saveDefaultEvent(event,offset,partition);
    ...
    private void saveLoginEvent(EventDTO event, long offset, int partition){
        logger.info("message received in listener: {} thread id: {}", event,Thread.currentThread().threadId());
        try {
    ...
    private void saveDefaultEvent(EventDTO event, long offset, int partition){
